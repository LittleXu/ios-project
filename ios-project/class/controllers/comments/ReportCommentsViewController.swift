...
class ReportCommentsViewController: BaseTableViewController {
    ...
    private func setupViews() {
        navigationItem.title = "举报评论"
        let bottomButton = setupBottomButton(title: "举报") { [weak self] in
            ...
            Common.fetch {
                Common.hideLoading()
                Common.showToast("举报成功! 将在审核通过后展示!")
                Common.saveCommentReply(comment_content: self.comment.comment_content, from: self.comment.from, content: text, isReport: true)
            }
        }
        bottomButton.setBackgroundImage(Color.error.asImage(), for: .normal)
    }
